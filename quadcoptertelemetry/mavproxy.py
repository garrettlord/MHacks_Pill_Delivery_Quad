...
class MPStatus(object):
    ...
    def __init__(self):
        ...
        self.setup_mode = start_setup_mode
        self.wp_op = None
    ...
    def show_new(self, f, pattern=None):
        '''write status to status.txt'''
        temp = ""
        if pattern is None:
            return temp
...
def cmd_status_new(args):
    ...
    if len(args) == 0:
        ...
        for pattern in args:
            temp += mpstate.status.show_new(temp, pattern=pattern)
    _print(temp )
    # returns raw input 
    # cmd_bat_new -> returns flight and avionics battery levels
...
def cmd_bat_new(args):
    '''show battery levels'''
    temp = "flightBattery : %u" % mpstate.status.battery_level + ", "
    temp += "avionicsBattery : %u" % mpstate.status.avionics_battery_level
    _print (temp)
...
def process_master(m):
    ...
    if m.first_byte and auto_protocol:
...
def process_mavlink(slave):
    ...
    try:
        ...
        return
    try:
        if slave.first_byte and auto_protocol:
...
def open_logs():
    ...
    if aircraft_name is not None:
        dirname = "%s/logs/%s" % (aircraft_name, time.strftime("%Y-%m-%d"))
        mkdir_p(dirname)
        highest = None
...
def process_hacks(cmd):
    if cmd == "exit":
        exit_mavproxy()
    if cmd == "get_info":
        print "getting info..."
        cmd_status_new(info_params) 
        print cmd_bat_new(0)
...
def main_loop():
    ...
    if not mpstate.status.setup_mode and not nowait:
        for master in mpstate.mav_master:
...
def mavproxy_main():
    ...
    global log_filename
    global use_qc_controls
    global start_setup_mode
    global enable_DTR
    global show_errors
    global speech
    global num_LIPOs
    ...
    global cmd
    global use_GUI_console
    global load_map
    global load_module
    global use_MAV09_protocol
    global auto_protocol
    global nowait
    global continue_logs
    global dialect
    ...
    target_system = 1
    target_component = 1
    log_filename = 'mav.tlog'
    use_qc_controls = False
    start_setup_mode = False
    enable_DTR = False
    show_errors = False
    speech = False
    num_LIPOs = 0
    aircraft_name = "Pegasus"
    cmd = None
    use_GUI_console = False
    load_map = False
    load_module = []
    use_MAV09_protocol = False
    auto_protocol = False
    nowait = False
    continue_logs = False
    ...
    # parser.add_option("--master",dest="master", action='append', help="MAVLink master port", default=["COM14"])
    ...
    mpstate.continue_mode = continue_logs
    ...
    if not comm_port:
        ...
        if len(serial_list) == 1:
            comm_port = [serial_list[0].device]
        else:
            print('''
    ...
    mpstate.status.target_component = target_component

    mpstate.mav_master = []

    # open master link
    for mdev in comm_port:
        if mdev.startswith('tcp:'):
            m = mavutil.mavtcp(mdev[4:])
        elif mdev.find(':') != -1:
            m = mavutil.mavudp(mdev, input=True)
        else:
            m = mavutil.mavserial(mdev, baud=baudrate, autoreconnect=True)
    ...
    for p in output:
        mpstate.mav_outputs.append(mavutil.mavlink_connection(p, baud=baud_rate, input=False))

    if SITL_port:
        mpstate.sitl_output = mavutil.mavudp(SITL_port, input=False)

    mpstate.settings.numcells = num_LIPOs
    mpstate.settings.speech = speech
    mpstate.settings.streamrate = streamrate
    mpstate.settings.streamrate2 = streamrate

    global msg_period, param_period, heartbeat_period, battery_period, heartbeat_check_period
    ...
    if 'HOME' in os.environ and not start_setup_mode:
        start_script = os.path.join(os.environ['HOME'], ".mavinit.scr")
        if os.path.exists(start_script):
            run_script(start_script)

    if aircraft_name is not None:
        start_script = os.path.join(aircraft_name, "mavinit.scr")
        if os.path.exists(start_script):
            run_script(start_script)
        else:
